...
│   ├── home-hero.php     Three featured slots plus a recent row
...
The homepage is being assembled section by section. Its first section is the non-inserter `patterns/home-hero.php` composition: three featured slots — a dominant lead beside two stacked companions — with a row of three recent posts beneath. **Sticky posts choose what fronts the publication.** Ticking "Stick to the top of the blog" on a post gives it a featured slot, in order; any slot left unchosen falls back to the most recent unfeatured post, so ticking nothing behaves as a newest-first hero and ticking three art-directs the whole thing. The hero never renders a hole. Slot resolution lives in `inc/hero.php` — see "Choosing what the hero features" below for the two Core constraints that shape it.

Each semantic `article` exposes the metadata appropriate to its role. The lead shows its image, the chip pair, title and a desktop excerpt, with its text overlaid on the photograph; the two companions show image, chip pair and title; the recent row shows the same without an excerpt. The hero intentionally has no pagination: it is a featured module, not the complete post list.
...
The palette was originally red-tinted, with three separate surface tokens shading warm, red-black, and cool charcoal. That was replaced because the accent and the grounds shared a hue: an accent sitting on tinted black never separated from it, and read muddy rather than sharp. Removing the tint from the surfaces is what makes the accent land, and it is why the lime swap needed nothing more than four palette values. Two tokens became unnecessary in the process and were deleted, taking the palette from ten colors to eight.
...
### Choosing what the hero features

Tick **Stick to the top of the blog** on a post and it takes a featured slot in the homepage hero. The first three sticky posts fill the three slots in order. Anything you leave unfilled takes the most recent unfeatured post, so the hero works whether you use the feature or ignore it entirely.

`inc/hero.php` computes the queries. Two Core constraints shape how, and both are easy to fall foul of:

**A Query block cannot be restricted to given post IDs.** The `include` key it accepts belongs to `taxQuery`, for terms; `post__in` is set only by `sticky: "only"`. Position is the only lever available, so each slot is addressed by counting — the first *n* slots count through the sticky posts, the rest count through the non-sticky ones. The offsets are computed from `get_option( 'sticky_posts' )` at render time, which is what makes them exact.

**Do not verify this with `get_posts()`.** It special-cases `include` by overriding `numberposts` and ignoring `offset` entirely. A check written with it reports duplicate posts in three of the five sticky states, and those duplicates are artefacts of the wrapper rather than defects in the queries. Use `WP_Query`, which is what the block builds.

The recent row uses `sticky: "ignore"` plus an explicit `exclude` of the featured three, rather than `sticky: "exclude"`. The latter bars *every* sticky post from the row, so marking five posts sticky would remove the fourth and fifth from the homepage altogether instead of merely not featuring them.
...
The theme carries nine distinct post-card treatments. The variety is deliberate — it is how the homepage signals rank — but from `0.42.0` they share one vocabulary so they read as one family without becoming one shape.

**Radius scale.** Four tokens on `:root` in `style.css`, and no other radius values anywhere:

| Token | Value | Applies to |
|---|---|---|
| `--parimaanam-radius-card` | `1rem` | filled card boxes, full-bleed overlay cards |
| `--parimaanam-radius-media` | `0.625rem` | images inside cards |
| `--parimaanam-radius-chip` | `0.375rem` | meta chips |
| `--parimaanam-radius-control` | `0.5rem` | inputs, buttons, selects |

`settings.border.radius` stays `false` in `theme.json`. That only hides the radius control from the block editor; theme CSS is unaffected. Leaving it off is what keeps the scale a scale rather than a suggestion.

**Chips.** `.parimaanam-chip` with an `--accent` (filled) or `--outline` variant carries every category and date label. Padding and background sit on the inner anchor rather than Core's wrapper, so the whole chip is the click target; Core renders `post-terms` as a wrapper holding one anchor per term, so a post in two categories gets two chips, and Core's `", "` separator is hidden because the flex gap already separates them.

**The filled box is scoped by orientation, not by page.** A box traces the outline of an image stacked *above* its text, which describes the archive listing grid and nothing else. Cards whose image sits *beside* their text — the homepage category pair, the sidebar's recent posts — are rows, and rows are separated by rules; stacked boxes in a half-width column read as clutter. The hero lead and the showcase feature card are full-bleed with text overlaid, and take the card radius on the card itself.

That last point is a trap worth knowing. Core applies the featured-image border radius to the inner `img`, not to the figure. On a full-bleed card that curves the photograph while the scrim above it stays square, and the scrim reaches `0.97` opacity at the bottom, so its corners stand proud of the image. Both overlay cards already carry `overflow: hidden` to contain their hover zoom, so the fix is to round the card and zero the image. Core's Latest Posts block is a different block entirely and `theme.json`'s featured-image radius does not reach it, so the sidebar thumbnails are handled explicitly in `style.css`.

**Listing type** runs at 18px titles and 15px excerpts, below the global scale, because in a list of nine results the reading sizes make each card claim more room than it earns. Titles are reduced by redefining the preset variable in scope, because Core's `has-*-font-size` classes carry `!important` and the alternative is answering `!important` with `!important`.

**Never apply `letter-spacing` to Tamil.** It visually detaches combining vowel signs (ெ, ா, ி) from the consonant they attach to, breaking the grapheme cluster a reader scans for. The theme has exactly one `letter-spacing` declaration, in `header.css`, and it is correct: it spaces Latin digits emitted by a CSS counter.

**Nine posts per archive page**, set by a `pre_get_posts` filter in `functions.php` rather than in Settings → Reading. The grid packs three columns, so ten leaves one card alone on the last row of every page. The count is a property of the layout, not the site — change the grid to two or four columns and nine is wrong — so it lives with the layout and travels in the theme zip. Homepage sections are unaffected; they run their own queries with `inherit` false and explicit per-page counts.

The listing grid's `minimumColumnWidth` is `20rem`, chosen by measurement: in the wide container a third column needs tracks of 395px or less and a fourth appears at 290px, so any value between those produces exactly three. `wideSize` caps the container at `80rem`, so no viewport can widen it enough to admit a fourth.
...
10. **Section-based magazine homepage:** the homepage begins with a six-post hero — three editor-chosen featured slots and three recent posts — powered by four small Core queries whose offsets are computed from the sticky-post option at render time. Separating these roles from the category modules keeps each query and editorial purpose explicit. It does not copy a third-party theme's branding, proprietary components, subscription features, sliders, or pop-ups.
